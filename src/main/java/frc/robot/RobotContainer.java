...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    // intake.setDefaultCommand(new IntakeCommand(intake, conveyor,
    // driverController));
  ...
  private void configureButtonBindings() {
    ...
    // Extend Climber
    new DirectionPadButton(driverController, Direction.UP).whileHeld(new InstantCommand(() -> climber.extend(1)))
        .whenReleased(new InstantCommand(() -> climber.hold()));
    // Retract Climber
    new DirectionPadButton(driverController, Direction.DOWN).whileHeld(new InstantCommand(() -> climber.retract(.50)))
        .whenReleased(new InstantCommand(() -> climber.hold()));
    // Roll Climb left
    new DirectionPadButton(driverController, Direction.LEFT).whileHeld(new InstantCommand(() -> climber.roll(.30)))
        .whenReleased(new InstantCommand(() -> climber.hold()));
    // Roll Climb Right
    new DirectionPadButton(driverController, Direction.RIGHT).whileHeld(new InstantCommand(() -> climber.roll(-.30)))
        .whenReleased(new InstantCommand(() -> climber.hold()));

    // operator controls
    // Extend Climber
    new DirectionPadButton(operatorController, Direction.UP).whileHeld(new InstantCommand(() -> climber.extend(1)))
        .whenReleased(new InstantCommand(() -> climber.hold()));
    // Retract Climber
    new DirectionPadButton(operatorController, Direction.DOWN).whileHeld(new InstantCommand(() -> climber.retract(.50)))
        .whenReleased(new InstantCommand(() -> climber.hold()));
    // Roll Climb left
    new DirectionPadButton(operatorController, Direction.LEFT).whileHeld(new InstantCommand(() -> climber.roll(.30)))
        .whenReleased(new InstantCommand(() -> climber.hold()));
    // Roll Climb Right
  ...
  public CommandBase getAutonomousCommand() {
     ...
     * AutoPathFactory.generateTrajectory(this.m_driveTrain); return
     * command.andThen(() -> m_driveTrain.tankDriveVolts(0, 0));
     */
  ...
  public void addAutonomousOptions() {
    ...
    this.chooser.addOption("Center Forward Low", new AutoDriveForwardCenterLow(m_driveTrain, shooter, conveyor));
